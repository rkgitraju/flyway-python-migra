...
def delete_generated_file(file_path):
    ...
    if os.path.exists(file_path):
        try:
            os.remove(file_path)
            print(f"🗑️ Successfully deleted failed migration file: {os.path.basename(file_path)}")
        except OSError as e:
...
def run_migra_and_generate_script():
    ...
    try:
        ...
        print(f"❌ ERROR: {e}. Check if 'migra' is in your system's PATH.")
        sys.exit(1)
    except subprocess.CalledProcessError as e:
    ...
    # 2. Check for changes
    if not diff_sql:
        print("✅ No schema changes detected. Exiting.")
        return None
